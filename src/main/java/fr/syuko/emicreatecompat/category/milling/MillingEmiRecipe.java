package fr.syuko.emicreatecompat.category.milling;

import com.simibubi.create.AllBlocks;
import dev.emi.emi.api.recipe.BasicEmiRecipe;
import dev.emi.emi.api.stack.EmiIngredient;
import dev.emi.emi.api.stack.EmiStack;
import dev.emi.emi.api.widget.WidgetHolder;
import fr.syuko.emicreatecompat.create.recipe.ChancedStack;
import fr.syuko.emicreatecompat.plugin.CreateEmiCategories;

public class MillingEmiRecipe extends BasicEmiRecipe {

    private static final int WIDTH = 177;

    private static final int HEIGHT = 53;

    private static final int INPUT_X = 14;

    private static final int INPUT_Y = 8;

    private static final int SINGLE_OUTPUT_X = 138;

    private static final int OUTPUT_X = 132;

    private static final int OUTPUT_Y = 26;

    private static final int OUTPUT_SPACING = 19;

    public MillingEmiRecipe(MillingDisplay display) {
        super(CreateEmiCategories.MILLING, display.id(), WIDTH, HEIGHT);

        inputs.add(EmiIngredient.of(display.input()));
        catalysts.add(EmiStack.of(AllBlocks.MILLSTONE.get()));
        for (ChancedStack output : display.outputs()) {
            outputs.add(EmiStack.of(output.stack()));
        }
    }

    @Override
    public void addWidgets(WidgetHolder widgets) {
        widgets.addDrawable(0, 0, WIDTH, HEIGHT, (draw, mouseX, mouseY, delta) -> MillingRender.draw(draw, 0, 0));
        widgets.addSlot(inputs.getFirst(), INPUT_X, INPUT_Y);

        boolean single = outputs.size() == 1;
        for (int i = 0; i < outputs.size(); i++) {
            int
                    x =
                    single
                    ? SINGLE_OUTPUT_X
                    : OUTPUT_X + (i % 2) * OUTPUT_SPACING;
            int y = OUTPUT_Y - (i / 2) * OUTPUT_SPACING;
            widgets.addSlot(outputs.get(i), x, y).recipeContext(this);
        }
    }
}
